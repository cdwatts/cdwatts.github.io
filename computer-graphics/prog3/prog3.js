...
function main() {
    ...
    canvas.onmousewheel = function(ev) {scaleObj(ev, gl, canvas);};
...
function loadedObject(n, c, v, i, fc, sc, fn, sn) {
    this.name = n;
    this.center = c;
    this.vert = v;
    this.ind = i;
...
function shadeToggle() {
    ...
    if (flatShaded === true) {
        ...
        if (buttonC){drawSOR(gl); drawLights(gl);}
...
function updateScreen(){  
    ...
    var obj = new loadedObject(name, center, vert, ind, flatColor, smoothColor, flatNormz, smoothNormz);
    objects.push(obj);
    drawObjects();
...
//*********************************************//
//scale function: scales the current SOR object//
//*********************************************//
function scaleObj(ev, gl, canvas){
